#!/bin/bash -l
#SBATCH --job-name=w2v2_B_lr_0004_ft   # Job name
#SBATCH --output=/finetune_wav2vec2_hf.o # Name of stdout output file
#SBATCH --error=/finetune_wav2vec2_hf.e  # Name of stderr error file
#SBATCH --partition=standard-g  # Partition (queue) name
#SBATCH --nodes=1              # Total number of nodes 
#SBATCH --exclusive
#SBATCH --cpus-per-task=32 
#SBATCH --gpus-per-node=8       # 
#SBATCH --time=0-18:59:00       # Run time (d-hh:mm:ss)
...
torchrun \
	--nproc_per_node 8 /scripts/finetuning/huggingface_run_speech_recognition_ctc_multigpu.py \
	--dataset_name="lp_100h" \
  --cache_dir="/cache" \
